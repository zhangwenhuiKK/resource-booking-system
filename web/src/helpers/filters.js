...
export const CategoryParams = [
  ...
  { name: "Etching", value: false },
  { name: "Mechanical Inspection", value: false },
  { name: "Electrical Inspection", value: false },
  { name: "Actuator Lab", value: false },
...
export const onFilterByCategory = (param, filteredData) => {
  if (param === "all") {
    return filteredData;
  } else {
  ...
  params.forEach((feature) => {
    if (feature.name === "macLab" && feature.value === true) {
      ...
      filteredData = filteredData.filter(
        (room) => room.assets.opWalls === true
      );
    } else if (feature.name === "projector" && feature.value === true) {
      filteredData = filteredData.filter(
        (room) => room.assets.projector === true
      );
    }
  });
  return filteredData;
...
export const onFilterByCapacity = (params, filteredData) => {
  ...
  params.forEach((capacity) => {
    if (capacity.value === true) {
      roomsByCapacity.push(
        ...filteredData.filter((room) => room.capacity === capacity.capacity)
      );
    }
  });
