...
class App extends Component {
  ...
  // Edits a booking from the database and updates the React state
  onEditBooking = (roomId, bookingId) => {
    this.setRoom(roomId);
  ...
  render() {
    ...
    const requireAuth = (Comp) => (signedIn ? <Comp /> : <LandingPage />);
    const LandingPage = () => {
      return !!decodedToken && signedIn ? (
        <BookingPage />
      ) : (
    ...
    return (
      <div id="app" className="App">
        <Routes>
          <Route path="/" exact element={requireAuth(LandingPage)} />
